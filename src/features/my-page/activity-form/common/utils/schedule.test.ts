...
describe('schedule utils', () => {
  ...
  describe('calculateEndTime', () => {
    it('시작 시간에 진행 시간을 더한 후 1분을 차감한 종료 시간을 반환한다', () => {
      // 09:00 + 2시간 = 11:00 -> 10:59
      expect(calculateEndTime('09:00', 2)).toBe('10:59');

      // 14:30 + 3시간 = 17:30 -> 17:29
      expect(calculateEndTime('14:30', 3)).toBe('17:29');
    });

    it('자정(24:00)에 도달하는 경우 23:59로 반환하여 익일 처리를 방지한다', () => {
      // 23:00 + 1시간 = 24:00 -> 23:59
      expect(calculateEndTime('23:00', 1)).toBe('23:59');
    });

    it('한 자리 수 시간일 경우 앞에 0을 붙여 2자리로 유지한다', () => {
      // 07:00 + 2시간 = 09:00 -> 08:59
      expect(calculateEndTime('07:00', 2)).toBe('08:59');
    });
  });
